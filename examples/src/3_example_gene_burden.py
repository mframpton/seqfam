...
cmc_result_df = cmc.do_multivariate_tests(sample_s, geno_df, group_col=gene_col, agg_col="pop_frq_cat", agg_cat_l=list(pop_frq_cat_dict.keys()), covar_df=covar_df, results_path=results_path)
#Uncomment the next line to run with the tests without the covariates.
#cmc_result_df = cmc.do_multivariate_tests(sample_s, geno_df, group_col=gene_col, agg_col="pop_frq_cat", agg_cat_l=list(pop_frq_cat_dict.keys()), covar_df=None, results_path=results_path)
